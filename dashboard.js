// =================================================================
// FICHIER JS PARTIE 1/2 : dashboard_part1.js (V3.2 Core & Init + Persistance)
// Contient constantes, variables d'état, calculs Geo/Astro/Kalman,
// FONCTIONS DE PERSISTANCE, et les fonctions d'initialisation des capteurs/systèmes.
// =================================================================
...
let kSpd = 0, kUncert = 1000; // État et Incertitude du filtre de Kalman
let lServH = null, lLocH = null; 
let als = null; 
let lastLux = null; 
let manualMode = null; // Mode Jour/Nuit Manuel
let netherMode = false; // Mode Nether
...
let externalBatteryLevel = 99.999; // Niveau de batterie externe fictif
...
/**
 * Ancienne fonction de filtre de Kalman (MAINTENANT DÉPASSÉE - LOGIQUE DÉPLACÉE VERS updateDisp).
 */
...
// ===========================================
// ... (Toutes les fonctions astro/midday restent inchangées)
...
// ===========================================
// ... (Toutes les fonctions d'initialisation restent inchangées)
...
// ===========================================
// ... (Toutes les fonctions de gestion du système restent inchangées, 
// sauf les références à stopGPS/startGPS/fastDOM qui sont dans Part 2)
...
// =================================================================
// FIN FICHIER JS PARTIE 1/2
// =================================================================
// =================================================================
// FICHIER JS PARTIE 2/2 : dashboard_part2.js (V3.2 Logic & DOM + EKF & Export TXT)
...
function fastDOM() {
    ...
    saveState(); // <<< Sauvegarde l'état toutes les secondes
...
/**
 * Gestionnaire principal de l'événement GPS watchPosition (Mise à jour EKF)
 */
function updateDisp(pos) {
    ...
    // Calcul de la vitesse GPS 3D brute et de l'accélération
    if (lPos && dt > 0.05) { // Utiliser un délai min pour une accélération stable
    ...
    const accellLong = dt > 0 ? (spd3D - lastSpd3D) / dt : 0; // Accélération calculée

    // --- NOUVELLE LOGIQUE EKF (PREDICTION + UPDATE) ---
    
    // 1. Prédiction (Utilise la dernière vitesse filtrée et l'accélération calculée)
    const fSpd_PREV = lPos ? (lPos.kalman_kSpd_LAST ?? kSpd) : 0;
    
    // Prédiction de l'état (v = v0 + a * dt)
    const kSpd_PREDICTED = fSpd_PREV + accellLong * dt;
    // Prédiction de l'incertitude (P = P + Q)
    const kUncert_PREDICTED = kUncert + Q_NOISE * dt;

    // 2. Mesure (Calcul de l'incertitude de la mesure R_dyn)
    ...
    // Nouvelle vitesse stable (kSpd)
    kSpd = kSpd_PREDICTED + K * (spd3D - kSpd_PREDICTED); // Correction
    
    // Nouvelle incertitude stable (kUncert)
    ...
    // Sauvegarde des états pour la prochaine itération
    lPos = pos; 
    lPos.speedMS_3D = spd3D; 
    lPos.timestamp = cTime; 
    lPos.speedMS_3D_LAST = spd3D; 
    lPos.kalman_R_val = R_dyn; 
    lPos.kalman_kSpd_LAST = fSpd; // Sauvegarde la vitesse filtrée pour la prédiction future
...
// ===========================================
// NOUVEL EXPORT TEXTE (Remplace la capture PNG)
// ===========================================

function collectDataForTextDownload() {
    const dataPanel = document.querySelectorAll('.data-panel');
    let textContent = `--- Rapport du Dashboard GNSS Quantique ---\n`;
    textContent += `Heure du rapport: ${new Date().toISOString()}\n`;
    textContent += `=========================================\n\n`;

    dataPanel.forEach((panel, index) => {
        const h2Elements = document.querySelectorAll('h2');
        let title = '';
        if (h2Elements[index]) {
            title = h2Elements[index].textContent;
        } else if (index === 0) {
        ...
        panel.querySelectorAll('.data-item').forEach(item => {
            const key = item.querySelector('span:first-child')?.textContent?.trim() || 'Clé Inconnue';
            const value = item.querySelector('span:last-child')?.textContent?.trim() || 'N/A';
            const cleanKey = key.replace(/[:\s]+$/, '');
            textContent += `${cleanKey.padEnd(25)}: ${value}\n`;
        });
        textContent += '\n';
    });

    // Ajout des stats de session persistantes
    textContent += `--- Statistiques de Session Persistantes ---\n`;
    textContent += `Distance Totale Démarrée (m): ${distM.toFixed(2)}\n`;
    textContent += `Vitesse Max Session (km/h): ${(maxSpd * KMH_MS).toFixed(5)}\n`;
    textContent += `Mode Nether (Ratio 1:8): ${netherMode ? "ACTIVÉ" : "DÉSACTIVÉ"}\n`;
    textContent += `-------------------------------\n`;
...
function captureScreenshot() {
    const textData = collectDataForTextDownload();
    const blob = new Blob([textData], { type: 'text/plain;charset=utf-8' });
    const link = document.createElement('a');
    
    if (navigator.msSaveOrOpenBlob) { 
        navigator.msSaveOrOpenBlob(blob, 'dashboard_data.txt');
    } else {
    ...
    alert("Données du tableau de bord téléchargées sous forme de texte (dashboard_data.txt).");
}
...
function setTarget() {
    ...
    // CORRECTION: Utilisation de l'ID correct 'cap-dest'
    if ($('cap-dest')) $('cap-dest').textContent = 'Calcul en cours...';
}

function initAll() { 
    // Initialisation et chargement de l'état
    loadState(); 
    resetDisp(false); // Réinitialise les variables GPS/Kalman mais garde l'état de session chargé
    syncH(); 
    initALS(); 
    initAdvancedSensors(); 
    ...
    // Mise à jour de l'affichage initial suite au chargement de l'état
    ...
    if ($('reset-all-btn')) $('reset-all-btn').addEventListener('click', () => { 
        if (confirm("Êtes-vous sûr de vouloir tout réinitialiser (Distance, Max, Cible) ?")) {
            stopGPS(true);
            resetDisp(true); // Passe à true pour effacer localStorage
        }
    });
