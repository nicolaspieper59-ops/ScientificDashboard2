...
// Version Finale Corrigée (v2025.12.06) - Alignement strict sur la structure HTML fournie
...
// Formatage des données exponentielles (pour physique et relativité)
...
// Utilise un proxy mock pour la météo et l'API de temps UTC pour la synchronisation.
...
((window) => {
    ...
    // Météo (Stockées en variables globales pour les calculs physiques)
    ...
    // =================================================================
    // BLOC 3 : MODÈLES SCIENTIFIQUES (PHYSIQUE, RELATIVITÉ, SVT)
    // =================================================================
    ...
    function calculateBioSVT(tempC, altM, humidity_perc, pressure_Pa) {
        ...
        // Point de Rosée (Formule Magnus-Tetens)
        ...
        // Température du Bulbe Humide (Approximation Stull)
        ...
        // Saturation O₂ (Approximation Altitudinale)
    ...
    /** MOCK UKF 21 ÉTATS (Nécessite math.min.js) */
    class ProfessionalUKF {
        constructor() {
            this.STATE_SIZE = 21;
            // Utilisez math.zeros si math.js est chargé, sinon utilisez un tableau simple
            this.x = (typeof math !== 'undefined') ? math.zeros(this.STATE_SIZE) : new Array(this.STATE_SIZE).fill(0);
        }
        
        // MOCK de la propagation (Prédiction)
        predict_propagation(dt, imu_accel) {
            if (typeof math !== 'undefined') {
                 // Simulation de mise à jour des états 0 (lat), 1 (lon), 2 (alt), 5 (vel_up)
                this.x = this.x.map((val, index) => {
                    if (index < 3) return val + (Math.random() - 0.5) * 1e-6; // Position 
                    if (index === 5) return imu_accel[2] * dt; // Vitesse verticale
                    return val;
                });
            }
        }
        
        // MOCK de la correction (Mise à jour)
        update_correction(gps_measurement) {
            if (typeof math !== 'undefined') {
                // Met à jour l'état alt avec une petite correction vers la mesure GPS
                const gpsAlt = gps_measurement[2];
                const currentAlt = this.x.get([2]);
                const newAlt = currentAlt + (gpsAlt - currentAlt) * 0.1; // Fusion simple
                this.x.set([2], newAlt);
            }
        }

        getState() {
            // Renvoie les états clés
    ...
    const handleDeviceMotion = (event) => {
        ...
        // Gyroscope et Magnétomètre (MOCK)
        const now = Date.now();
        ...
        if (typeof DeviceOrientationEvent.requestPermission === 'function') {
            DeviceOrientationEvent.requestPermission().then(state => {
                 if (state === 'granted') {
                    ...
                    if ($('statut-capteur')) $('statut-capteur').textContent = 'Actif (IMU/Mag)';
                 }
            });
        } else {
            window.addEventListener('devicemotion', handleDeviceMotion);
    ...
    const handleGPS = (position) => {
        ...
        if (typeof turf !== 'undefined' && distMTotal > 0.0 || (newLat !== 0 && newLon !== 0)) { 
            ...
            // Fallback très basique si turf.js n'est pas là
        ...
        if ($('gps-acquisition-status')) $('gps-acquisition-status').textContent = 'Acquisition OK';
    };

    const handleGPSError = (error) => {
        console.error(`[GPS ERREUR] Code ${error.code}: ${error.message}`);
        if ($('statut-gps')) $('statut-gps').textContent = 'ERREUR GPS';
        if ($('gps-acquisition-status')) $('gps-acquisition-status').textContent = 'ERREUR ACQUISITION';
    };
    ...
    const syncH = async () => {
        ...
        try {
            ...
            if ($('date-heure-utc')) $('date-heure-utc').textContent = `N/A`;
        } finally {
    ...
    const fetchWeather = async (lat, lon) => {
        try {
            // MOCK de la réponse API (pour le test hors ligne)
            const mockData = {
                temp: TEMP_SEA_LEVEL_K + (Math.random() - 0.5) * 10, // Température en Kelvin autour de 15°C
                pressure: BARO_ALT_REF_HPA + (Math.random() - 0.5) * 10, // Pression en hPa
                humidity: 50 + (Math.random() - 0.5) * 20, // Humidité en %
    ...
    const updateSensorsAndFilter = () => {
        ...
        if (dt > 0) {
            ...
            // Force G Verticale (Accélération nette + 1G pour la gravité)
            ...
            // Les champs magnétiques sont N/A dans la structure, mais gardent les IDs pour le futur
            if ($('champ-mag-x')) $('champ-mag-x').textContent = 'N/A';
            ...
            if ($('pourcentage-vitesse-son')) $('%-vitesse-son').textContent = dataOrDefault(machNumber * 100, 2, ' %');
        } 
    ...
    const updateDOMSlow = () => {
        ...
        if (speedSamples > 0 && totalMoveTimeS > 0) {
            if ($('vitesse-moyenne-mvt')) $('vitesse-moyenne-mvt').textContent = dataOrDefault((totalSpeedSum / speedSamples) * KMH_MS, 2, ' km/h');
        }
        if (elapsedTime > 0) {
            if ($('vitesse-moyenne-totale')) $('vitesse-moyenne-totale').textContent = dataOrDefault((distMTotal / elapsedTime) * KMH_MS, 2, ' km/h');
        }
        ...
        if ($('distance-s-lumiere')) $('distance-s-lumiere').textContent = dataOrDefaultExp(distance_seconds, 2, ' s');
        if ($('distance-min-lumiere')) $('distance-min-lumiere').textContent = dataOrDefaultExp(distance_seconds / 60, 2, ' min');
        if ($('distance-h-lumiere')) $('distance-h-lumiere').textContent = dataOrDefaultExp(distance_seconds / 3600, 2, ' h');
        if ($('distance-j-lumiere')) $('distance-j-lumiere').textContent = dataOrDefaultExp(distance_seconds / 86400, 2, ' j');
        ...
        if (typeof SunCalc !== 'undefined') {
            ...
            if ($('azimut-soleil')) $('azimut-soleil').textContent = dataOrDefault((sunPos.azimuth * R2D + 180) % 360, 2, '°');
            if ($('duree-jour')) $('duree-jour').textContent = dataOrDefault((sunTimes.sunset.getTime() - sunTimes.sunrise.getTime()) / 3600000, 2, ' h');
            if ($('lever-soleil')) $('lever-soleil').textContent = sunTimes.sunrise.toLocaleTimeString('fr-FR');
            if ($('coucher-soleil')) $('coucher-soleil').textContent = sunTimes.sunset.toLocaleTimeString('fr-FR');
            
            if ($('phase-lune')) $('phase-lune').textContent = getMoonPhaseName(moonIllum.phase); 
            if ($('illumination-lune')) $('illumination-lune').textContent = dataOrDefault(moonIllum.fraction * 100, 1, ' %');
            if ($('altitude-lune')) $('altitude-lune').textContent = dataOrDefault(moonPos.altitude * R2D, 2, '°');
            if ($('azimut-lune')) $('azimut-lune').textContent = dataOrDefault((moonPos.azimuth * R2D + 180) % 360, 2, '°');
            if ($('distance-terre')) $('distance-terre').textContent = dataOrDefault(moonPos.distance / 1000, 0, ' km'); 
        }
        ...
        if (lastKnownWeather) {
            ...
            if ($('humidite-absolue-sim')) $('humidite-absolue-sim').textContent = dataOrDefault(bioSim.absoluteHumidity, 3, ' g/m³');
    ...
    const handleControlEvents = () => {
        // Mettre à jour les IDs des contrôles pour correspondre à la structure HTML (si nécessaire)
        ...
        if ($('mode-nether-toggle')) $('mode-nether-toggle').addEventListener('click', () => {
            ...
            if ($('rapport-distance-alt-nether')) $('rapport-distance-alt-nether').textContent = netherMode ? '8.000' : '1.000';
        });
        
        // Initialiser la masse
        if ($('masse-objet')) $('masse-objet').addEventListener('input', (e) => {
    ...
    const initDashboard = () => {
        ...
        // 2. Initialisation de la Carte (Leaflet - MOCK)
        ...
        if (typeof math !== 'undefined') {
            ...
            if ($('statut-ekf-fusion')) $('statut-ekf-fusion').textContent = 'Initialisation...';
        } else {
            console.error("math.min.js non chargé. Le filtre UKF est désactivé.");
            // Le dashboard restera en mode "GPS Brute"
        }
