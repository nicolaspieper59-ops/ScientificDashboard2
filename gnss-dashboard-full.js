// =================================================================
// FICHIER 1/2 : gnss-dashboard-part1.js
// Contient: Constantes, Variables d'État Globales, Utilitaires, EKF, Astro
// =================================================================
...
const MIN_SPD = 0.0000001; // MODIFIÉ: Autorise 1 mm/s
const ZVU_ACTIVATION_THRESHOLD_S = 1.0; // AJOUTÉ: Temps pour verrouillage ZVU
const R_GPS_DISABLED = 100000.0; 
const VEL_NOISE_FACTOR = 4.5; 
const IMU_NOISE_FLOOR = 0.05; 
...
/** Calcule le bruit de mesure dynamique R (Modèle Écholocation / Flux Optique) */
function getKalmanR(acc, alt, pressure) { 
    ...
    // Facteur de Vitesse/Énergie Cinétique (Réduction du bruit R aux hautes vitesses)
    const kSpd_squared = kSpd * kSpd;
    const speedFactor = 1 / (1 + MASS_PROXY * kSpd_squared); 

    // Boost R aux basses vitesses (non-ZVU)
...
// ===========================================
// FONCTIONS ASTRO UTILITAIRES (Saisons/TST)
// ===========================================
...
/** Calcule le Temps Solaire Vrai (TST) et la Déclinaison Solaire (Saisons Réelles). */
function getSolarTime(date, lon) {
    if (date === null || lon === null) return { TST: 'N/A', MST: 'N/A', EOT: 'N/D', ECL_LONG: 'N/D', DECL: 'N/D', TST_MS: 0 }; 
    const d = toDays(date);
    const M = solarMeanAnomaly(d); 
    const L = eclipticLongitude(M); 
    
    // Obliquité (Inclinasion axiale, source des saisons)
    const epsilon = D2R * (23.4393 - 0.000000356 * d); 
    
    // Déclinaison Solaire (Saisons / Anti-Minecraft)
    ...
    return { TST: toTimeString(tst_ms), TST_MS: tst_ms, MST: toTimeString(mst_ms), EOT: eot_min.toFixed(3), ECL_LONG: final_ecl_long.toFixed(2), DECL: final_declination.toFixed(2) };
}

// Autres utilitaires (pour la météo)
...
// =================================================================
// FICHIER 2/2 : gnss-dashboard-part2.js
// Contient: Boucle Principale, IMU, Météo, Carte, Événements
// Dépend de toutes les constantes et fonctions globales de gnss-dashboard-part1.js
// =================================================================
...
function updateDisp(pos) {
    ...
    const accOverride = parseFloat($('gps-accuracy-override')?.value || 0);
    if (accOverride > 0) { effectiveAcc = accOverride; }

    let spdH = spd_raw_gps ?? 0; 
    const dt = lPos ? (cTimePos - lPos.timestamp) / 1000 : MIN_DT;

    // 1. Facteur d'Amortissement IMU (Dampening)
    ...
    // 3. VITESSE 3D INSTANTANÉE BRUTE (Pré-EKF)
    let spdV_raw = 0; 
    ...
    // 4. LOGIQUE ZÉRO-VITESSE DYNAMIQUE (ZVU) AVEC TEMPORISATION
    const GPS_NOISE_SPEED = effectiveAcc / VEL_NOISE_FACTOR; 
    const isStaticByIMU = latestLinearAccelAccelMagnitude < STATIC_ACCEL_THRESHOLD;
    const isStaticByDynamicSpeed = spd3D_raw < GPS_NOISE_SPEED;
    const isStatic_raw = isStaticByIMU && isStaticByDynamicSpeed; 
    
    if (isStatic_raw) {
        zvu_timer_s += dt;
        if (zvu_timer_s >= ZVU_ACTIVATION_THRESHOLD_S) { zvu_is_locked = true; }
    } else {
        zvu_timer_s = 0.0;
        zvu_is_locked = false; 
    }

    const isStatic = zvu_is_locked;
    ...
    if (isStatic) {
        ...
        kSpd = 0.0; kUncert = 0.000001; 
        lastFSpeed = 0.0; 
        
        if (zvu_timer_s > 60) { 
            kUncert = 0.0000001; kAltUncert = 0.0000001; 
        }
    }
    ...
    // 6. VITESSE VERTICALE et HORIZONTALE
    let spdV = 0; 
    if (lPos && lPos.kAlt_old !== undefined && dt > MIN_DT && alt !== null) { 
    ...
    // CALCUL DE LA DENSITÉ DE L'AIR & TRAÎNÉE
    let airDensity = "N/A";
    let dragForce = "N/A"; 
    ...
    if (tempCMatch) {
        ...
        if (!isNaN(tempK) && tempK > 0 && pressurePa > 0) {
            ...
            const FRONTAL_AREA = 1.0; 
            const density = parseFloat(airDensity);
            dragForce = (0.5 * density * sSpdFE ** 2 * DRAG_COEFF * FRONTAL_AREA).toFixed(2);
        }
    }


    // --- MISE À JOUR DU DOM (Physique & Biomimétisme) ---
    ...
    if ($('drag-force')) $('drag-force').textContent = dragForce + (dragForce !== "N/A" ? ' N' : ''); 
    
    // Affichage du Statut ZVU (Temporisation)
    if ($('zvu-lock-status')) $('zvu-lock-status').textContent = isStatic ? '🔒 Verrouillé' : 'Ouvert';
    if ($('zvu-lock-time')) $('zvu-lock-time').textContent = zvu_timer_s.toFixed(1) + ' s';
    ...
    if (isSubterranean) {
        ...
        if (isStatic) {
             ...
             statusText += ` | 🔒 ZVU LOCK (${zvu_timer_s.toFixed(1)} s) (Seuil Dyn: ${(GPS_NOISE_SPEED_DISPLAY * KMH_MS).toFixed(2)} km/h)`;
    ...
    lPos = pos; lPos.timestamp = cTimePos; lPos.kAlt_old = kAlt_new; 
}


// ===========================================
// FONCTIONS ASTRONOMIE (MISE À JOUR DU DOM)
// ===========================================

/** Met à jour l'affichage de l'Astro, incluant Déclinaison Solaire et Angle Zénithal. */
function updateAstro(latA, lonA) {
    ...
    const zenithalAngle = (90 - sunElevationDeg); // Angle Zénithal (Anti-Minecraft)
    ...
    if ($('zenith-angle')) $('zenith-angle').textContent = `${zenithalAngle.toFixed(2)} °`; // AJOUTÉ
    if ($('sun-declination')) $('sun-declination').textContent = DECL + ' °'; // AJOUTÉ (Saisons Réelles)

    $('tst').textContent = formatTime(TST_minutes);
    $('lsm').textContent = formatTime(TST_minutes - EOT_minutes); 
    $('noon-solar').textContent = solarNoonLocal.toTimeString().split(' ')[0] + ' Local'; 
...
function updateMinecraftClock(TST_minutes, elevation) {
    ...
    // ... (Logique d'affichage des phases de lune) ...
}

// ===========================================
// FONCTIONS API MÉTÉO & IMU
// ===========================================
...
async function updateWeather(latA, lonA) {
    ...
    try {
        ...
        // ... (Calcul des directions et affichage des métriques météo) ...
...
function handleDeviceMotion(event) {
    ...
    // ... (Logique de détection de gravité G_STATIC_REF et calcul des accélérations linéaires)
    ...
    if (Math.abs(kAccel_mag - G_ACC) < ACCEL_MOVEMENT_THRESHOLD) { 
        G_STATIC_REF.x = kAccel.x; G_STATIC_REF.y = kAccel.y; G_STATIC_REF.z = kAccel.z;
...
function initMap(latA, lonA) { /* ... (Logique d'initialisation de Leaflet) ... */ }
function updateMap(latA, lonA) { /* ... (Logique de mise à jour de Leaflet) ... */ }
...
function startGPS() {
    if (wID === null) { setGPSMode(currentGPSMode); sTime = null; }
}
function stopGPS(resetButton = true) {
    if (wID !== null) { navigator.geolocation.clearWatch(wID); wID = null; }
    if (resetButton) { if ($('toggle-gps-btn')) $('toggle-gps-btn').textContent = `▶️ MARCHE GPS`; }
}
function emergencyStop() { emergencyStopActive = true; stopGPS(false); }
function resumeSystem() { emergencyStopActive = false; startGPS(); }
function handleErr(err) { /* ... (Logique d'erreur GPS) ... */ }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- Initialisation des contrôles (Sélection d'environnement, boutons...) ---
    ...
    // ... (Logique des écouteurs de boutons) ...
    ...
    // --- Intervalle lent pour les mises à jour du DOM ---
    if (domID === null) {
        domID = setInterval(() => {
            const now = getCDate();
            if (now) {
                $('local-time').textContent = now.toLocaleTimeString();
                // ... (Mise à jour d'autres éléments d'horloge)
            }
