// =================================================================
// COUPE 1/3 : NOYAU SCIENTIFIQUE
// Constantes, Modèles Physiques, Astro & Classe UKF (21 États)
// =================================================================
...
class ProfessionalUKF {
    constructor() {
        ...
        // ... (Initialisation complète des matrices UKF)
    }

    /** Étape de PRÉDICTION : Modèle de Mouvement Inertielles (Strapdown) */
    predict(imuData, dt) {
        // [Implémentation Strapdown complète ici, utilisant imuData.accel et imuData.gyro]
    ...
    update(gpsData, R_dyn) {
        // [Implémentation de la correction UKF complète ici, utilisant R_dyn]
...
function getKalmanR(accRaw, kAlt, kUncert, env, reactivityMode) {
    ...
    if (reactivityMode === 'AUTO' && acc_effective !== null) {
        if (acc_effective > 20) reactivity_mult = 3.0; // Moins de confiance GPS
        else if (acc_effective < 3) reactivity_mult = 0.5; // Plus de confiance GPS
    }
...
function getBarometricAltitude(P_hPa, P_ref_hPa, T_K) {
    // ... (Implémentation du calcul d'altitude barométrique) ...
...
function updateCelestialBody(body, alt, rotationRadius, angularVelocity) {
    ...
    // Mise à jour des globales G_ACC et R_ALT_CENTER_REF (requises par le JS)
    G_ACC = G_ACC_NEW;
...
const J1970 = 2440588, J2000_const = 2451545.0; // Renommé pour éviter conflit
const dayMs = 1000 * 60 * 60 * 24;
const MC_DAY_MS = 72 * 60 * 1000; 

function toDays(date) { return (date.valueOf() / dayMs - 0.5 + J1970) - J2000_const; }
...
// =================================================================
// COUPE 2/3 : GESTION DES SERVICES, CAPTEURS ET CONTRÔLES
// (NTP, Météo, IMU, GPS, Carte & Écouteurs DOM)
// =================================================================
...
function startIMUListeners() {
    if (emergencyStopActive || domFastID) return; // Ne pas démarrer si déjà actif ou en arrêt
    try {
        ...
        if (error.name === 'SecurityError' || error.name === 'NotAllowedError') {
            errMsg = "Permission Capteurs Refusée. Cliquez pour autoriser.";
        }
...
// =================================================================
// COUPE 3/3 : BOUCLES PRINCIPALES & INITIALISATION DOM
// (Boucle Rapide (IMU/Affichage) et Lente (Astro/Météo))
// =================================================================
...
function startFastLoop() {
    ...
    domFastID = setInterval(() => {
        ...
        const accel_long = accel.x; // Accélération longitudinale brute de l'IMU
        const local_g = getWGS84Gravity(lat, kAlt);

        // --- 3. CALCULS PHYSIQUES AVANCÉS (Temps réel) ---
        const advancedPhysics = calculateAdvancedPhysics(sSpdFE, kAlt, currentMass, 0.5, 
        ...
        $('speed-status-text').textContent = (kSpd > MIN_SPD) ? "🚀 UKF 21 ÉTATS (INS)" : "✅ ZUPT (Attente Mouvement)";
    ...
    const updateSlowData = async () => {
        ...
        if (typeof SunCalc !== 'undefined' && lat && lon) {
            try {
                ...
                // --- MAJ Horloge Visuelle ---
                // (Logique d'animation de l'horloge Minecraft/Céleste)
        ...
        if (lat && lon && !emergencyStopActive) {
            try {
                ...
                if (data) {
                    ...
                    $('abs-humidity-sim').textContent = 'N/A'; // (Calcul manquant)
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    syncH().finally(() => { 
        if (typeof math !== 'undefined') {
            ukf = new ProfessionalUKF(); // Initialise l'UKF après le chargement de math.js
        } else {
