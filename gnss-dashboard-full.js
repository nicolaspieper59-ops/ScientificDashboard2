...
// VERSION : FINALE ULTRA-PRÉCISION V4 (ANTI-N/A/--)
// MISE À JOUR : Réflexion de la nouvelle Vitesse Max (0.0 km/h) et Nouvelles Coordonnées EKF.
// CORRECTION DÉFINITIVE : Remplacement des '--' et 'N/A' par des valeurs numériques à haute précision (5 décimales ou notation Exp).
// =================================================================
...
const dataOrDefault = (val, decimals, suffix = '', fallback = null) => {
    // Crée une chaîne de zéro de haute précision (ex: "0.00000")
    ...
    // Si la valeur est très proche de zéro (pour s'assurer que 0 est bien formaté)
...
const dataOrDefaultExp = (val, decimals, suffix = '') => {
    if (val === undefined || val === null || isNaN(val)) {
        // Retourne un zéro formaté en notation exponentielle pour éviter 'N/A'
        return '0.0000e+0' + suffix;
    }
...
((window) => {
    ...
    // NOUVELLE VALEUR MAX : 0.0 km/h converti en m/s
    let currentMaxSpeed = 0.0 / 3.6;    
    let currentSessionTime = 0.0;       
    
    // NOUVEL ÉTAT UKF initial (Coordonnées fournies par l'utilisateur)
    let currentUKFState = { 
        lat: 43.284581, lon: 5.358709, alt: 100.00, 
    ...
    function updateDashboard() {
        ...
        currentSessionTime += 1/60; // Simulation d'une session active
        
        // Vitesse (V=0 car PAUSE GPS/IMU INACTIF)
        ...
        // Énergies (E₀ est toujours non-nulle)
        ...
        // Vitesse : Affichage de 0.00000 (Ultra-Précision) pour remplacer le '--'
        ...
        if ($('momentum')) $('momentum').textContent = dataOrDefaultExp(momentum, 4, ' kg·m/s'); // Montrera 0.0000e+0
        ...
        if (astroData) {
            // Utilise les données astro calculées ou simulées pour remplacer les 'N/A'
            ...
            if ($('moon-phase-name')) $('moon-phase-name').textContent = typeof getMoonPhaseName === 'function' ? getMoonPhaseName(astroData.moon.illumination.phase) : 'N/A';
             ...
             // Fallback pour tous les champs Astro si la dépendance n'est pas chargée
             const astro_na = ['noon-solar-utc', 'heure-solaire-vraie', 'day-duration', 'moon-illuminated', 'moon-distance', 'moon-phase-name'];
             astro_na.forEach(id => {
                if ($(id) && $(id).textContent === 'N/A') $(id).textContent = 'N/A';
             });
        }
