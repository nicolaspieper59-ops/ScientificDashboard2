...
const MIN_SPD = 0.00001; // 👈 CORRIGÉ : Seuil final pour forcer la vitesse à 0 (m/s)
const R_GPS_DISABLED = 100000.0; // R élevé pour désactiver mathématiquement le GPS (Mode IMU-Only)
const VEL_NOISE_FACTOR = 4.5; // 👈 CORRIGÉ : Augmenté pour un seuil ZVU plus strict
...
const STATIC_ACCEL_THRESHOLD = 0.05; // 👈 CORRIGÉ : Seuil d'accélération IMU (m/s²)
...
function updateDisp(pos) {
    ...
    if (isStatic) {
        ...
        // CORRECTION CRITIQUE ZVU: Hard reset EKF
        kSpd = 0.0;
        kUncert = 0.000001; // 👈 CORRIGÉ : Incertitude très faible
        lastFSpeed = 0.0; 
    }
    ...
    // 👈 CORRECTION : Utilisation de spd3D (brut) pour les métriques, selon la demande.
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if ($('reset-all-btn')) $('reset-all-btn').addEventListener('click', () => { 
        ...
        if (confirm("Êtes-vous sûr de vouloir tout réinitialiser? (Distance, Max, Kalman)")) { 
            ...
            if (tracePolyline) tracePolyline.setLatLngs([]); // Réinitialise la trace de la carte
        } 
    });
    
    if ($('data-capture-btn')) $('data-capture-btn').addEventListener('click', () => {
        // Logique de capture de données (à implémenter si nécessaire)
    ...
    // --- Intervalle lent pour les mises à jour (Astro/Temps) si le GPS est lent/absent ---
    if (domID === null) {
        domID = setInterval(() => {
            // Mettre à jour l'heure et la date locales même sans GPS
            const now = getCDate();
            if (now) {
