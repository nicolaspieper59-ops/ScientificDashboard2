...
// Version Finale Corrigée V2 (v2025.12.06) - ALIGNEMENT ID CRITIQUE
...
((window) => {
    ...
    // =================================================================
    // BLOC 5 : BOUCLES D'EXÉCUTION ET MISE À JOUR DU DOM (CORRECTIONS ID)
    // =================================================================
    ...
    const updateSensorsAndFilter = () => {
        ...
        if (dt > 0) {
            ...
            const dragForce = dynamicPressure * 1.2 * 0.5;
            ...
            // Vitesse
            if ($('vitesse-3d-instantanee')) $('vitesse-3d-instantanee').textContent = dataOrDefault(instSpd_mps * KMH_MS, 2, ' km/h'); // Corrigé
            if ($('vitesse-brute-mps')) $('vitesse-brute-mps').textContent = dataOrDefault(instSpd_mps, 3, ' m/s'); // Corrigé
            
            // IMU
            if ($('acceleration-x')) $('acceleration-x').textContent = dataOrDefault(lastAccX, 3, ' m/s²'); // Corrigé
            if ($('acceleration-y')) $('acceleration-y').textContent = dataOrDefault(lastAccY, 3, ' m/s²'); // Corrigé
            if ($('acceleration-z')) $('acceleration-z').textContent = dataOrDefault(lastAccZ, 3, ' m/s²'); // Corrigé
            
            // UKF States & Debug (Correspondance aux IDs EKF/UKF)
            if ($('latitude-ekf')) $('latitude-ekf').textContent = dataOrDefault(ukfState.lat, 6, '°'); // Corrigé
            if ($('longitude-ekf')) $('longitude-ekf').textContent = dataOrDefault(ukfState.lon, 6, '°'); // Corrigé
            if ($('altitude-ekf')) $('altitude-ekf').textContent = dataOrDefault(kAlt, 2, ' m'); // Corrigé
            ...
            if ($('pourcentage-vitesse-son')) $('pourcentage-vitesse-son').textContent = dataOrDefault(machNumber * 100, 2, ' %'); // Corrigé
        } 
    ...
    const updateDOMSlow = () => {
        ...
        // --- ASTRO (SunCalc - CORRECTION DES IDs) ---
        ...
        if (typeof SunCalc !== 'undefined') {
            ...
            // Correction des IDs selon index (6).html
            ...
            if ($('moon-distance')) $('moon-distance').textContent = dataOrDefault(moonPos.distance / 1000, 0, ' km'); 
        }
    ...
    const handleControlEvents = () => {
        // Supposons que ces IDs sont dans l'HTML pour les contrôles
        if ($('marche-gps-btn')) $('marche-gps-btn').addEventListener('click', () => { // Ajout de l'ID pour le bouton
