...
((window) => {
    ...
    // NOTE: Ceci est conservé pour le DOM, mais la valeur 'phase' doit venir de vos librairies.
    ...
    const updateDOMSlow = () => {
        ...
        // ... (Logique Temps / Distance / Relativité) ...
        ...
        // VÉRIFIEZ QUE VOS LIBRAIRIES EXPOSENT UNE FONCTION GLOBALE NOMMÉE EXACTEMENT COMME CELA
        if (typeof calculateAstroDataHighPrec === 'function') {
            
            try {
                // Cette fonction DOIT être définie dans lib/ephem.js ou lib/astro.js
                // et doit retourner l'objet ci-dessous (voir section 3.1)
                const astroData = calculateAstroDataHighPrec(now, currentLat, currentLon);
                ...
                if ($('day-duration')) $('day-duration').textContent = dataOrDefault((astroData.sun.sunset.getTime() - astroData.sun.sunrise.getTime()) / 3600000, 2, ' h');
                if ($('sunrise-times')) $('sunrise-times').textContent = astroData.sun.sunrise.toLocaleTimeString('fr-FR');
                if ($('sunset-times')) $('sunset-times').textContent = astroData.sun.sunset.toLocaleTimeString('fr-FR');
                ...
                if (astroData.moon.times.alwaysUp) {
                    moonTimesText = 'Toujours visible';
                } else if (astroData.moon.times.alwaysDown) {
                    moonTimesText = 'Jamais visible';
                } else {
                    const rise = astroData.moon.times.rise ? astroData.moon.times.rise.toLocaleTimeString('fr-FR') : '--:--';
                    const set = astroData.moon.times.set ? astroData.moon.times.set.toLocaleTimeString('fr-FR') : '--:--';
                ...
                console.error("🔴 ERREUR: L'appel à calculateAstroDataHighPrec a échoué. Vérifiez vos librairies.", e);
                if ($('sun-alt')) $('sun-alt').textContent = 'N/A (Erreur Astro)';
                if ($('moon-phase-name')) $('moon-phase-name').textContent = 'N/A (Erreur Astro)';
            }

        } else {
            console.warn("🟡 AVERTISSEMENT: La fonction calculateAstroDataHighPrec() est introuvable. Les données Astro sont indisponibles.");
            if ($('sun-alt')) $('sun-alt').textContent = 'N/A (Fonction Astro manquante)';
            if ($('moon-phase-name')) $('moon-phase-name').textContent = 'N/A (Fonction Astro manquante)';
        }
