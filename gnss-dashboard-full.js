...
function updateDisp(pos) {
    ...
    if (tempCMatch) {
        ...
        if (!isNaN(tempK) && tempK > 0 && pressurePa > 0) {
            ...
            lastAirDensity = parseFloat(airDensity);
        }
    }

    // CALCUL FORCE DE TRAÎNÉE (DRAG)
    const A = 0.5;
    const Cd = 1.0;
    const dragForce = 0.5 * lastAirDensity * sSpdFE ** 2 * Cd * A;
...
function handleDeviceMotion(event) {
    ...
    if (Math.abs(kAccel_mag - G_ACC) < ACCEL_MOVEMENT_THRESHOLD) { 
        ...
        linear_z = kAccel.z - G_STATIC_REF.z; // Accélération verticale projetée sur z
...
function setGPSMode(mode) {
    ...
    currentGPSMode = mode;
    // Redémarre la surveillance GPS avec les nouvelles options de fréquence
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if ($('reset-all-btn')) $('reset-all-btn').addEventListener('click', () => { 
        ...
        if (confirm("Êtes-vous sûr de vouloir tout réinitialiser? (Distance, Max, Kalman)")) { 
            // Réinitialisation de l'état global et des filtres
