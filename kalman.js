// ========================
// kalman.js
// Filtre Kalman et gestion GPS + capteurs
// ========================

// ========================
// PARAMÈTRES DU FILTRE DE KALMAN
// ========================
const KALMAN_Q = 0.001;      // bruit du modèle
const KALMAN_P_INIT = 10;    // confiance initiale réaliste
const VITESSE_MIN = 0.3;     // seuil minimal de vitesse (m/s)
...
function haversine3D(p1, p2) {
    ...
    const Δφ = toRad(p2.latitude), Δλ = toRad(p2.longitude - p1.longitude);
...
function calculerVitesse3D(pos) {
    ...
    positionPrecedente = pos;
    let vitesse = dist / dt;
    if (vitesse < VITESSE_MIN) vitesse = 0; // éviter les valeurs absurdes
    return { vitesse, distance: dist, dt };
}
...
function miseAJour(pos) {
    ...
    // Affichage
    safeSetText('vitesse-kalman', `Vitesse Filtrée : ${v_kalman_kmh.toFixed(2)} km/h`);
    safeSetText('vitesse-raw', `Vitesse GPS : ${(v_raw_ms*3.6).toFixed(2)} km/h`);
    ...
    safeSetText('temps', `Temps : ${tempsDebut ? ((now - tempsDebut)/1000).toFixed(2) : '0.00'} s`);
    ...
    safeSetText('kalman-error', `Erreur P : ${kalmanState.P.toFixed(2)}`);
...
function arreterCockpit() {
    ...
    safeSetText('vitesse-raw','Vitesse GPS : -- km/h');
    safeSetText('vitesse-max','Vitesse max : 0.00 km/h');
    safeSetText('distance','Distance : 0.000 km');
    safeSetText('gps','GPS : --');
    safeSetText('kalman-gain', '--');
    safeSetText('kalman-error', '--');
    safeSetText('accel-z','0.00 m/s²');
}
