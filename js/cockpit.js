...
function afficherVitesse(v) {
    ...
    const mmps = mps*1000;
    safeSetText('vitesse',
        `Temps : ${new Date().toLocaleTimeString()} | Vitesse instantanée : ${v.toFixed(2)} km/h | Moyenne : ${moyenne.toFixed(2)} km/h | Max : ${vitesseMax.toFixed(2)} km/h | ${mps.toFixed(2)} m/s | ${mmps.toFixed(0)} mm/s`
    );
}

function afficherDistance() {
    const km = distanceTotale/1000, m = distanceTotale, mm = m*1000;
    const secLumiere = m/299792458;
    const al = secLumiere/(3600*24*365.25);
...
function afficherPourcentage(v) {
    const mps = v/3.6;
    const pctLumiere = (mps/299792458)*100;
    const pctSon = (mps/343)*100;
    safeSetText('pourcentage', `% Lumière : ${pctLumiere.toExponential(2)}% | % Son : ${pctSon.toFixed(2)}%`);
}

function afficherGPS(g) {
    safeSetText('gps',
        `Latitude : ${g.latitude.toFixed(6)} | Longitude : ${g.longitude.toFixed(6)} | Altitude : ${(g.altitude??'--')} | Précision GPS : ${g.accuracy??'--'}%`
    );
}
...
async function activerCapteurs() {
    ...
    if ('AmbientLightSensor' in window) {
        try {
            ...
            sensor.addEventListener('reading', () => appendText('capteurs', ` | Lumière : ${Math.round(sensor.illuminance)} lux`));
            sensor.start();
        ...
        if (typeof DeviceOrientationEvent.requestPermission==='function') {
            try {
                ...
                if (permission==='granted') {
                    window.addEventListener('deviceorientation', e => appendText('capteurs', ` | Niveau à bulle : ${e.beta?.toFixed(1)}°`));
                }
            } catch(e){ console.warn(e); }
        } else {
            window.addEventListener('deviceorientation', e => appendText('capteurs', ` | Niveau à bulle : ${e.beta?.toFixed(1)}°`));
...
function afficherMedaillon() {
    ...
    // Soleil
    ctx.fillStyle='#ffd700'; ctx.beginPath(); ctx.arc(210,90,8,0,2*Math.PI); ctx.fill();
    ctx.fillStyle='#fff'; ctx.font='10px monospace'; ctx.fillText('☀️ Soleil',200,80);
    // Lune
    ctx.fillStyle='#ccc'; ctx.beginPath(); ctx.arc(90,110,6,0,2*Math.PI); ctx.fill();
